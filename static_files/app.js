		...
		<div id='settingsContainer' className='center centerCross'>
			<div id='settings' className='col center centerCross'>
				...
				<select onChange={(e)=>{
					...
					{['', 'male', 'female'].map((s)=>(
						<option key={s} selected={s==preferredGenderState}>{s}</option>
					))}
				</select>
...
class Icons {
	static heart = <svg className='heart' width="23px" height="20px" viewBox="0 0 23 20" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
		<g id="Desktop" transform="translate(-38.000000, -200.000000)">
			<g id="sidebar" transform="translate(0.000000, 168.000000)">
				<g id="title" transform="translate(0.000000, 21.000000)" fill="#444444">
	...
	static paw = <svg className='paw' width="26px" height="23px" viewBox="0 0 26 23" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlnsXlink="http://www.w3.org/1999/xlink">
		<g id="-" stroke="none" strokeWidth="1" fill="none" fillRule="evenodd">
			<g id="Desktop" transform="translate(-35.000000, -289.000000)">
				<g id="sidebar" transform="translate(0.000000, 168.000000)">
					<g id="favourite" transform="translate(0.000000, 84.000000)">
						<g id="paw-solid" transform="translate(35.000000, 37.000000)" fill="#DFCDD6" fillRule="nonzero">
...
function Profile({
			profile: {name, surname, location, age, imageSrc, quote},
			removeProfile,
			toggleFav,
			isFav
		}){
	...
	<div className='profileContainer row center centerCross'>
		...
		<button className='closeButton' onClick={removeProfile}>{Icons.cross}</button>

		<div className='profile col'>

            <img 
                className='profilePicture' 
                alt={`${name} ${surname} animal profile picture.`}
                src={imageSrc}
            />
			...
			<div className='profileInfo'>
				...
				<div className='lower'>
					<img id='quotationMark' src='./quotes.png'/>
...
function App() {
	...
	let newProfile = async () => {
		let profileObj = await ProfileCreator.getProfileObj();
	...
	React.useEffect(()=>{
		...
		setTimeoutState(window.setTimeout(async ()=>{
			await newProfile();
			setTimeoutState(-1);
		}, interval*1000)); //interval is in seconds

		$('.slideshowImages').scrollTop(0);
	}, [timeoutState]);
	...
	}, []) // this empty arg means this effect will run once on mount

	return (
		<div className="App">
			<header className='row centerCross'>
				<img className='logo' src='matchmeowLogo.svg'/>
			...
			<section className='row'>
				...
				{(isMobile() && !hideFav) ? null : <Slideshow slideshowElements={
			...
			{(isShowSettings) ? <Settings hideSettings={()=>setShowSettings(false)}/> : null}
		</div>
	);
}
